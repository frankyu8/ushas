...
import org.apache.spark.sql.catalyst.lineage.RelationColumn
import org.apache.spark.sql.catalyst.plans.logical.Project

object SparkLineageExample {
  def main(args: Array[String]) {
    ...
    // 这里是第一步获取逻辑计划
    val logic_plan = a.parsePlan("select * from (select substr(a+1,0,1) as c,a+3 as d  from (select 1 as a,2 as b))")
    println(logic_plan)
    ...
    println(111)
    println(resolved_plan.lineageChildren)
