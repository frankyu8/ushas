...
class Dataset[T] private[sql](
  ...
  /**
   * 这里会把create view command 转化成 view 的 relation  例如:Map(abc -> Relation[age#6L,name#7] json
   * println(sparkSession.sessionState.catalog.tempViews)
   * 重复命名视图都会在这步进行调整
   * step1
   * Map(abc -> Relation[age#6L,name#7] json
   * )
   * step2
   * Map(abc -> Project [age#6L, name#7]
   * +- SubqueryAlias `abc`
