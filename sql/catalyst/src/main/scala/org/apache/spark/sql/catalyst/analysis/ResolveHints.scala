...
object ResolveHints {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperatorsUp {
      case h: UnresolvedHint if BROADCAST_HINT_NAMES.contains(h.name.toUpperCase(Locale.ROOT)) =>
        if (h.parameters.isEmpty) {
          ...
          ResolvedPlan.logMatch("ResolveHints.ResolveBroadcastHints", h)
  ...
  object ResolveCoalesceHints extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperators {
      case h: UnresolvedHint if COALESCE_HINT_NAMES.contains(h.name.toUpperCase(Locale.ROOT)) =>
        ResolvedPlan.logMatch("ResolveHints.ResolveCoalesceHints", h)
  ...
  object RemoveAllHints extends Rule[LogicalPlan] {
    //    这里是偏函数的概念 List(1,2,3) map {case i:Int=>i+1}   res:List(Int)=List(2,3,4)
    def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperatorsUp {
      case h: UnresolvedHint =>
        ResolvedPlan.logMatch("ResolveHints.RemoveAllHints", h)
        h.child
    }
  }
