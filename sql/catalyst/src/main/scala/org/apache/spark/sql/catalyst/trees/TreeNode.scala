...
/**
 * The definition TreeNode[BaseType <: TreeNode[BaseType]] says:
 * TreeNode is typed such that the type parameter BaseType is at least (in the sense of sub-classing) also a TreeNode[BaseType].
 * Speaking roughly that means: The type-parameter must also be a TreeNode itself.
 */
// scalastyle:off
