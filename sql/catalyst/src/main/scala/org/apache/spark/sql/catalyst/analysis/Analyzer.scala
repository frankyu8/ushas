...
import org.apache.spark.sql.catalyst.print.ResolvedPlan
...
//  建立尾部规则
  ...
  object CTESubstitution extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case u@With(child, relations) =>
        ResolvedPlan.logMatch("CTESubstitution", u)
        substituteCTE(child, relations.foldLeft(Seq.empty[(String, LogicalPlan)]) {
          case (resolved, (name, relation)) =>
            resolved :+ name -> executeSameContext(substituteCTE(relation, resolved))
        })
      case other =>
        ResolvedPlan.logMatch("CTESubstitution", other)
        other
    }
  ...
  object WindowsSubstitution extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      ...
      case w@WithWindowDefinition(windowDefinitions, child) =>
        ResolvedPlan.logMatch("WindowsSubstitution", w)
  ...
  object ResolveAliases extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case a@Aggregate(groups, aggs, child) if child.resolved && hasUnresolvedAlias(aggs) =>
        ResolvedPlan.logMatch("ResolveAliases", a)
        Aggregate(groups, assignAliases(aggs), child)

      case g: GroupingSets if g.child.resolved && hasUnresolvedAlias(g.aggregations) =>
        ResolvedPlan.logMatch("ResolveAliases", g)
        g.copy(aggregations = assignAliases(g.aggregations))

      case p@Pivot(groupByOpt, pivotColumn, pivotValues, aggregates, child)
        if child.resolved && groupByOpt.isDefined && hasUnresolvedAlias(groupByOpt.get) =>
        ResolvedPlan.logMatch("ResolveAliases", p)
        Pivot(Some(assignAliases(groupByOpt.get)), pivotColumn, pivotValues, aggregates, child)

      case p@Project(projectList, child) if child.resolved && hasUnresolvedAlias(projectList) =>
        ResolvedPlan.logMatch("ResolveAliases", p)
  ...
  object ResolveGroupingAnalytics extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperatorsUp {
      case a if !a.childrenResolved =>
        ResolvedPlan.logMatch("ResolveGroupingAnalytics", a)
        a // be sure all of the children are resolved.

      // Ensure group by expressions and aggregate expressions have been resolved.
      case a@Aggregate(Seq(c@Cube(groupByExprs)), aggregateExpressions, child)
      ...
      case s@Sort(order, _, child)
        ...
        ResolvedPlan.logMatch("ResolveGroupingAnalytics", s)
        val groupingExprs = findGroupingExprs(child)
  ...
  object ResolvePivot extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
      case p: Pivot if !p.childrenResolved || !p.aggregates.forall(_.resolved)
        ...
        ResolvedPlan.logMatch("ResolvePivot", p)
        p
  ...
  object ResolveRelations extends Rule[LogicalPlan] {
    ...
    //    这里是apply方法，进行树的深度遍历，只要match到了就进行Unresolve的解析，解析成已经resolve的状态
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case i@InsertIntoTable(u: UnresolvedRelation, parts, child, _, _) if child.resolved =>
        ResolvedPlan.logMatch("ResolveRelations", i)
      ...
      case u: UnresolvedRelation =>
        ResolvedPlan.logMatch("ResolveRelations", u)
        resolveRelation(u)
    }
  ...
  object ResolveReferences extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case p: LogicalPlan if !p.childrenResolved =>
        ResolvedPlan.logMatch("ResolveReferences", p)
        p
      ...
      case a: Aggregate if containsStar(a.aggregateExpressions) =>
        ResolvedPlan.logMatch("ResolveReferences", a)
      ...
      case t: ScriptTransformation if containsStar(t.input) =>
        ResolvedPlan.logMatch("ResolveReferences", t)
        t.copy(
      ...
      case g: Generate if containsStar(g.generator.children) =>
        ResolvedPlan.logMatch("ResolveReferences", g)
        failAnalysis("Invalid usage of '*' in explode/json_tuple/UDTF")

      // To resolve duplicate expression IDs for Join and Intersect
      case j@Join(left, right, _, _) if !j.duplicateResolved =>
        ResolvedPlan.logMatch("ResolveReferences", j)
        j.copy(right = dedupRight(left, right))
      case i@Intersect(left, right, _) if !i.duplicateResolved =>
        ResolvedPlan.logMatch("ResolveReferences", i)
        i.copy(right = dedupRight(left, right))
      case e@Except(left, right, _) if !e.duplicateResolved =>
        ResolvedPlan.logMatch("ResolveReferences", e)
        e.copy(right = dedupRight(left, right))
      // When resolve `SortOrder`s in Sort based on child, don't report errors as
      // we still have chance to resolve it based on its descendants
      case s@Sort(ordering, global, child) if child.resolved && !s.resolved =>
        ResolvedPlan.logMatch("ResolveReferences", s)
        val newOrdering =
      ...
      case plan if containsDeserializer(plan.expressions) =>
        ResolvedPlan.logMatch("ResolveReferences", plan)
        plan

      case q: LogicalPlan =>
        ResolvedPlan.logMatch("ResolveReferences", q)
  ...
  object ResolveOrdinalInOrderByAndGroupBy extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case p if !p.childrenResolved =>
        ResolvedPlan.logMatch("ResolveOrdinalInOrderByAndGroupBy", p)
        p
      // Replace the index with the related attribute for ORDER BY,
      // which is a 1-base position of the projection list.
      case s@Sort(orders, global, child)
        if orders.exists(_.child.isInstanceOf[UnresolvedOrdinal]) =>
        ResolvedPlan.logMatch("ResolveOrdinalInOrderByAndGroupBy", s)
      ...
      case a@Aggregate(groups, aggs, child) if aggs.forall(_.resolved) &&
        ...
        ResolvedPlan.logMatch("ResolveOrdinalInOrderByAndGroupBy", a)
  ...
  object ResolveAggAliasInGroupBy extends Rule[LogicalPlan] {
    ...
    override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case agg@Aggregate(groups, aggs, child)
        ...
        ResolvedPlan.logMatch("ResolveAggAliasInGroupBy", agg)
      ...
      case gs@GroupingSets(selectedGroups, groups, child, aggs)
        ...
        ResolvedPlan.logMatch("ResolveAggAliasInGroupBy", gs)
        gs.copy(
  ...
  object ResolveMissingReferences extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      ...
      case sa@Sort(_, _, child: Aggregate) =>
        ResolvedPlan.logMatch("ResolveMissingReferences", sa)
        sa

      case s@Sort(order, _, child)
        if (!s.resolved || s.missingInput.nonEmpty) && child.resolved =>
        ResolvedPlan.logMatch("ResolveMissingReferences", s)
      ...
      case f@Filter(cond, child) if (!f.resolved || f.missingInput.nonEmpty) && child.resolved =>
        ResolvedPlan.logMatch("ResolveMissingReferences", f)
  ...
  object ResolveFunctions extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case q: LogicalPlan =>
        ResolvedPlan.logMatch("ResolveFunctions", q)
  ...
  object ResolveSubquery extends Rule[LogicalPlan] with PredicateHelper {
    ...
    private def resolveSubQuery(
      ...
      var current = e.plan
      do {
        // Try to resolve the subquery plan using the regular analyzer.在这里会用常规的execute进行解析
        previous = current
    ...
    private def resolveSubQueries(plan: LogicalPlan, plans: Seq[LogicalPlan]): LogicalPlan = {
      plan transformExpressions {
        case s@ScalarSubquery(sub, _, exprId) if !sub.resolved =>
          //       这里的ScalarSubquery(_, _, exprId)预留了两个通配符传递到  f: (LogicalPlan, Seq[Expression]) => SubqueryExpression)
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      ...
      case f@Filter(_, a: Aggregate) if f.childrenResolved =>
        ResolvedPlan.logMatch("ResolveSubquery", f)
        resolveSubQueries(f, Seq(a, a.child))
      // Only a few unary nodes (Project/Filter/Aggregate) can contain subqueries.
      case q: UnaryNode if q.childrenResolved =>
        ResolvedPlan.logMatch("ResolveSubquery", q)
  ...
  object ResolveSubqueryColumnAliases extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case u@UnresolvedSubqueryColumnAliases(columnNames, child) if child.resolved =>
        ...
        ResolvedPlan.logMatch("ResolveSubqueryColumnAliases", u)
  ...
  object GlobalAggregates extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperators {
      case p@Project(projectList, child) if containsAggregates(projectList) =>
        ResolvedPlan.logMatch("GlobalAggregates", p)
  ...
  object ResolveAggregateFunctions extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case f@Filter(cond, agg@Aggregate(grouping, originalAggExprs, child)) if agg.resolved =>
        ResolvedPlan.logMatch("ResolveAggregateFunctions", f)
      ...
      case sort@Sort(sortOrder, global, aggregate: Aggregate) if aggregate.resolved =>
        ResolvedPlan.logMatch("ResolveAggregateFunctions", sort)
  ...
  object ExtractGenerator extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case p@Project(projectList, _) if projectList.exists(hasNestedGenerator) =>
        ResolvedPlan.logMatch("ExtractGenerator", p)
      ...
      case p@Project(projectList, _) if projectList.count(hasGenerator) > 1 =>
        ResolvedPlan.logMatch("ExtractGenerator", p)
      ...
      case g: Generate =>
        ResolvedPlan.logMatch("ExtractGenerator", g)
        g
  ...
  object ResolveGenerate extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case g: Generate if !g.child.resolved || !g.generator.resolved =>
        ResolvedPlan.logMatch("ResolveGenerate", g)
        g
  ...
  object ExtractWindowExpressions extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperatorsDown {
      ...
      case f@Filter(condition, _) if hasWindowFunction(condition) =>
        ResolvedPlan.logMatch("ExtractWindowExpressions", f)
      ...
      case p: LogicalPlan if !p.childrenResolved =>
        ResolvedPlan.logMatch("ExtractWindowExpressions", p)
        p
      ...
      case a@Aggregate(groupingExprs, aggregateExprs, child)
        if hasWindowFunction(aggregateExprs) &&
          a.expressions.forall(_.resolved) =>
        ResolvedPlan.logMatch("ExtractWindowExpressions", a)
        val (windowExpressions, aggregateExpressions) = extract(aggregateExprs)
        // Create an Aggregate operator to evaluate aggregation functions.
        val withAggregate = Aggregate(groupingExprs, aggregateExpressions, child)
  ...
  object ResolveRandomSeed extends Rule[LogicalPlan] {
    ...
    override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case p if p.resolved =>
        ResolvedPlan.logMatch("ResolveRandomSeed", p)
        p
      case p =>
        ResolvedPlan.logMatch("ResolveRandomSeed", p)
  ...
  object ResolveWindowOrder extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan resolveExpressions {
      case w@WindowExpression(wf: WindowFunction, spec) if spec.orderSpec.isEmpty =>
      ...
      case w@WindowExpression(rank: RankLike, spec) if spec.resolved =>
  ...
  object ResolveNaturalAndUsingJoin extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case j@Join(left, right, UsingJoin(joinType, usingCols), _)
        ...
        ResolvedPlan.logMatch("ResolveNaturalAndUsingJoin", j)
  ...
  object ResolveOutputRelation extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperators {
      case append@AppendData(table, query, isByName)
        ...
        ResolvedPlan.logMatch("ResolveOutputRelation", append)
  ...
  object ResolveDeserializer extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case p if !p.childrenResolved =>
        ResolvedPlan.logMatch("ResolveDeserializer", p)
        p
      case p if p.resolved =>
        ResolvedPlan.logMatch("ResolveDeserializer", p)
        p
  ...
  object ResolveNewInstance extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case p if !p.childrenResolved =>
        ResolvedPlan.logMatch("ResolveNewInstance", p)
        p
      case p if p.resolved =>
        ResolvedPlan.logMatch("ResolveNewInstance", p)
        p
  ...
  object ResolveUpCast extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case p if !p.childrenResolved =>
        ResolvedPlan.logMatch("ResolveUpCast", p)
        p
      case p if p.resolved =>
        ResolvedPlan.logMatch("ResolveUpCast", p)
        p
...
object EliminateUnions extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
    case u@Union(children) if children.size == 1 =>
      ResolvedPlan.logMatch("EliminateUnions", u)
      children.head
  }
}
...
object TimeWindowing extends Rule[LogicalPlan] {
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
    case p: LogicalPlan if p.children.size == 1 =>
      ResolvedPlan.logMatch("TimeWindowing", p)
